...
def actual_effect(X_train, y_train, X_test, subset_to_remove, original_value, target="linear"):
    reduced_X_train = np.delete(X_train, subset_to_remove, axis=0)
    reduced_y_train = np.delete(y_train, subset_to_remove, axis=0)
    reduced_value = target_value(reduced_X_train, reduced_y_train, X_test, target)
...
def actual(X_train, y_train, X_test, k=5, job_n=50, target="linear"):
    ...
    for subset_size in range(1, k + 1):
        ...
        best_k_score = Parallel(n_jobs=job_n)(delayed(actual_effect)(X_train, y_train, X_test, subset_to_remove, original_value, target) for subset_to_remove in subset_combinations)

        sort_subset_combinations = np.array(combinations_list)[np.argsort(best_k_score)[::-1]]
        best_subset[subset_size - 1] = sort_subset_combinations[0][0]
