...
class CNN(nn.Module):
    ...
    def get_individual_output(self, test_loader):
        ...
        model_output_tensor = torch.cat(model_output_list, dim=0)
    ...
    def get_individual_loss(self, test_loader):
        ...
        for images, labels in test_loader:
            images, labels = images.to(device), labels.to(device)
            raw_logit = self(images)

            loss_fn = nn.CrossEntropyLoss(reduction='none')
            logp = -loss_fn(raw_logit, labels)
...
if __name__ == "__main__":
    ...
    for ensemble_idx in range(args.ensemble):
        ...
        torch.save(model.state_dict(), f"./checkpoint/seed_{args.seed}_ensemble_{ensemble_idx}.pt")
