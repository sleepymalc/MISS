...
def actual(X_train, y_train, x_test, k=10, target=target):
	...
	for subset_size in range(1, k + 1):
		...
		best_reduced_Z_fix_test[subset_size - 1] = np.max(scores) # BUG likely is a bug
...
def IWLS_influence(X, x_test, y, coef, W, leverage=True, target=target):
	n_samples = X.shape[0]
...
def IWLS(X_train, y_train, x_test, target=target):
	...
	influences = IWLS_influence(X_train_bar, x_test_bar, y, coefficients, W)
...
def adaptive_IWLS(X_train, y_train, x_test, target=target):    
...
def margin(X_train, y_train, target=target):
...
# First-order method
def first_order(X_train, y_train, x_test, y_test, target=target):
	lr = LogisticRegression(penalty=None).fit(X_train, y_train)
...
# Best Subset
best_subset_combination, best_subset_fix_test, best_reduced_Z_fix_test = actual(X_train, y_train, x_test, k)
...
# Adaptive IWLS
adaptive_IWLS_best_k = adaptive_IWLS(X_train, y_train, x_test)
with open(out_file, 'a') as f:
